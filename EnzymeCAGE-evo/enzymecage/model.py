...
def calculate_pocket_weights(enzyme_coords_batch, enzyme_coords_mask):
    # Compute the geometric center of valid pocket residues.
...
class EnzymeCAGE(BaseModel):
    def __init__(
        ...
        self.use_msa_mean = use_msa and (use_msa if use_msa_mean is None else use_msa_mean)
        self.use_msa_node = use_msa and (use_msa if use_msa_node is None else use_msa_node)
        ...
        # Project MSA features to the ESM embedding dimension.
        if self.use_msa_mean or self.use_msa_node:
            self.msa_proj = nn.Linear(self.msa_dim, self.esm_dim)

            # Learn the relative contribution of ESM and MSA features.
            self.gate_layer = nn.Sequential(
                nn.Linear(self.esm_dim * 2, self.esm_dim),
                nn.Sigmoid()
            )
    ...
    def forward(self, data):
        ...
        if self.use_structure:
            ...
            node_feature = data['protein'].esm_node_feature  # [N, 1920]

            # Separate the concatenated ESM and MSA representations.
            esm_node = node_feature[:, :self.esm_dim]

            if self.use_msa_node:
                msa_node = node_feature[:, self.esm_dim:]

                # Project MSA features to the ESM dimension.
                msa_node_proj = self.msa_proj(msa_node)

                # Apply feature-wise gated fusion.
                gate_input = torch.cat([esm_node, msa_node_proj], dim=-1)
                gate = self.gate_layer(gate_input)

                fused_node = gate * msa_node_proj + (1 - gate) * esm_node
            else:
                fused_node = esm_node

            # Concatenate sequence and GVP structure representations.
            enzyme_output = torch.cat([gvp_output, fused_node], dim=-1)
        ...
        if self.use_esm:
            ...
            if self.use_msa_mean:
                msa_embedding, _ = to_dense_batch(data.msa_feature, data.msa_feature_batch)

                msa_proj = self.msa_proj(msa_embedding)
                gate_input = torch.cat([esm_embedding, msa_proj], dim=-1)
                gate = self.gate_layer(gate_input)

                fused_mean = gate * msa_proj + (1 - gate) * esm_embedding
                all_features.append(fused_mean)
            else:
