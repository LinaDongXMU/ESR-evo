...
def inference(model_conf):
    ...
    if model_conf.model == 'EnzymeCAGE':
        ...
        msa_node_feature = torch.load(model_conf.msa_node_feature) if use_msa_node else {}
        infer_dataset = load_geometric_dataset(model_conf.data_path, 
                                               ...
                                               getattr(model_conf, 'msa_mean_feature', None),
    ...
    if model_conf.model == 'EnzymeCAGE':
        ...
        available_proteins = set(protein_gvp_feat.keys()) & set(esm_node_feature.keys())
        if use_msa_node:
            available_proteins = available_proteins & set(msa_node_feature.keys())
        df_data = df_data[df_data[UID_COL].isin(available_proteins)]
    ...
    for model_name in tqdm(model_name_list):
        ...
        df_data = infer_dataset.df_data.copy()
        df_data['pred'] = preds.cpu().numpy()
