...
def check_files(model_conf):
    ...
    if getattr(model_conf, "use_msa", False):
        if getattr(model_conf, "use_msa_mean", True):
            assert hasattr(model_conf, "msa_mean_feature")
            assert os.path.exists(model_conf.msa_mean_feature)
        if getattr(model_conf, "use_msa_node", True):
            assert hasattr(model_conf, "msa_node_feature")
            assert os.path.exists(model_conf.msa_node_feature)
...
def calc_rxn_center_fp(rxn_center):
    # The reaction center is used as the matching template.
