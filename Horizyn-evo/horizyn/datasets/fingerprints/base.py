...
class BaseFingerprintDataset(WrapperDataset[K]):
    ...
    def __init__(
        ...
        standardize_error_policy: str = "raise",
        fingerprint_error_policy: str = "raise",
        ...
        **kwargs,
        ...
        Args:
            ...
            standardize_error_policy: What to do when reaction standardization fails.
                Options are "raise" and "warn_original". Defaults to "raise".
            fingerprint_error_policy: What to do when fingerprint generation fails.
                Options are "raise" and "zero". Defaults to "raise".
        ...
        allowed_standardize_policies = {"raise", "warn_original"}
        allowed_fingerprint_policies = {"raise", "zero"}
        if standardize_error_policy not in allowed_standardize_policies:
            raise ValueError(
                f"standardize_error_policy must be one of {allowed_standardize_policies}, "
                f"got {standardize_error_policy!r}"
            )
        if fingerprint_error_policy not in allowed_fingerprint_policies:
            raise ValueError(
                f"fingerprint_error_policy must be one of {allowed_fingerprint_policies}, "
                f"got {fingerprint_error_policy!r}"
            )
        self.standardize_error_policy = standardize_error_policy
        self.fingerprint_error_policy = fingerprint_error_policy
        self._warned_standardization_keys: set[K] = set()
        self._warned_fingerprint_keys: set[K] = set()
    ...
    def __getitem__(self, key: K) -> torch.Tensor:
        ...
        try:
            ...
            if self.standardize_error_policy != "warn_original":
                raise
            if key not in self._warned_standardization_keys:
                warnings.warn(
                    f"Reaction standardization failed for '{key}'. "
                    f"Using the original reaction SMILES. Error: {e}",
                    RuntimeWarning,
                    stacklevel=2,
                )
                self._warned_standardization_keys.add(key)

        # Generate and cache fingerprint
        try:
            fingerprint = self._generate_fingerprint(reaction_info)
        except Exception as e:
            if self.fingerprint_error_policy != "zero":
                raise Exception(f"Failed to generate fingerprint for reaction '{key}': {e}") from e
            if key not in self._warned_fingerprint_keys:
                warnings.warn(
                    f"Fingerprint generation failed for reaction '{key}'. "
                    f"Using a zero vector for this fingerprint block. Error: {e}",
                    RuntimeWarning,
                    stacklevel=2,
                )
                self._warned_fingerprint_keys.add(key)
            fingerprint = torch.zeros(self.vec_dim, dtype=self.dtype)
